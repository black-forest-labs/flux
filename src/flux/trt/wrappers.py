...
class Optimizer:
    ...
    def fuse_mha_qkv_int8_sq(self):
        ...
        qs = sorted(
                (x.group(0) for x in filter(lambda x: x is not None, [re.match(q_pat, key) for key in keys]))
            )
        ks = sorted(
                (x.group(0) for x in filter(lambda x: x is not None, [re.match(k_pat, key) for key in keys]))
            )
        vs = sorted(
                (x.group(0) for x in filter(lambda x: x is not None, [re.match(v_pat, key) for key in keys]))
            )
